...
namespace MapOfModes
{
	class FFT
	{
		public static double[] GetFFT(double[] originalFunction)
		{
			...
			double[] origCut = originalFunction.Take(nPoints).ToArray();
			Complex[] resultComplex = origCut.Select(x => new Complex(x, 0.0)).ToArray();
			Fourier.Forward(resultComplex, FourierOptions.Matlab);
			double[] result = resultComplex.Select(x => Math.Abs(x.Real)).ToArray();
			return result;
		}
	}
}